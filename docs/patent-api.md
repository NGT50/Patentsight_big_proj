...
| --- | --- | --- | --- | --- | --- | --- |
| Create Patent | 출원 생성 (임시 저장 상태로 생성) | POST | /api/patents | { "title", "type", "file_ids": [], "cpc", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [] } | { "patent_id", "applicant_id", "status" } | 🔹 status: "DRAFT" 자동 설정<br>🔹 type: "PATENT", "UTILITY_MODEL", "DESIGN", "TRADEMARK" |
| Get Patent Detail | 출원 상세 정보 조회 | GET | /api/patents/{patent_id} | – | { "patent_id", "applicant_id", "title", "type", "status", "attachments": [], "cpc", "application_number", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [], "classification_codes": [] } | 🔹 attachments: FileAttachment 기준<br>🔹 classification_codes: AI 분류 결과 포함 가능 |
...
| Get Document Versions | 출원 문서 버전 목록 조회 | GET | /api/patents/{patent_id}/document-versions | – | [ { "patent_id", "version_id", "version_no", "author_id", "change_summary", "is_current", "created_at" } ] | 🔹 SpecVersion 기반 버전 이력 |
| Get Latest Document | 최신 문서 내용 조회 (에디터용) | GET | /api/patents/{patent_id}/document/latest | – | { "version_no", "document": { "patent_id", "applicant_id", "title", "type", "status", "attachments": [], "cpc", "application_number", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [], "classification_codes": [] }, "updated_at" } | 🔹 에디터 최초 로딩 시 사용 |
| Update Document Content | 문서 내용 단순 수정 (버전 없음) | PATCH | /api/patents/{patent_id}/document | { "document": { "title", "type", "file_ids": [], "cpc", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [] } } | { "version_no", "document": { "patent_id", "applicant_id", "title", "type", "status", "attachments": [], "cpc", "application_number", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [], "classification_codes": [] }, "updated_at" } | 🔹 임시 저장 용도로만 사용 (주의) |
| Create Document Version | 새 문서 버전 생성 (내용 포함) | POST | /api/patents/{patent_id}/document-versions | { "new_document": { "title", "type", "file_ids": [], "cpc", "inventor", "technical_field", "background_technology", "invention_details": { "problem_to_solve", "solution", "effect" }, "summary", "drawing_description", "claims": [] }, "author_id", "change_summary" } | { "patent_id", "version_id", "version_no", "is_current" } | 🔹 생성 시 이전 버전 is_current=false 처리 |
| Update Version Info | 버전 정보(요약 등) 수정 | PATCH | /api/document-versions/{version_id} | { "change_summary", "is_current" } | { "patent_id", "version_id", "updated_at" } | 🔹 is_current=true 지정 시 기존 버전들 비활성화 |
| Restore Document Version | 특정 버전 복원 → 새 버전 생성 | POST | /api/document-versions/{version_id}/restore | – | { "patent_id", "version_id", "new_version_no", "restored_from" } | 🔹 기존 버전 내용을 복사해 새 버전 생성 |
...
| Get Document Versions | 출원 문서 버전 목록 조회 | GET | /api/document-versions?patent_id={patent_id} | – | [ { "patent_id", "version_id", "version_no", "author_id", "change_summary", "is_current", "created_at" } ] | SpecVersion 테이블 구조 반영 |
